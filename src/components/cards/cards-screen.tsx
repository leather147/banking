...
function CardArtwork({ card, controls, showNumber, brandName, hideBalance, frozenLabel, productLabel }: { card: BankCardRecord; controls: CardControls; showNumber: boolean; brandName: string; hideBalance: boolean; frozenLabel: string; productLabel: string }) {
  ...
  return <div className={cn("relative aspect-[1.62/1] overflow-hidden rounded-[1.5rem] border border-white/15 p-6 text-white", backgrounds[controls.style])}><div className="flex justify-between"><div><p className="max-w-56 truncate text-xs uppercase tracking-[.18em] text-white/60">{brandName} {controls.cardName}</p><p className="mt-1 font-bold">{productLabel}</p></div><ScanLine className="size-7" /></div><p className="number-display mt-7 text-2xl font-bold">{hideBalance ? "•••••• ₽" : card.balance.toLocaleString("ru-RU") + " ₽"}</p><div className="absolute inset-x-6 bottom-6"><p className="font-mono text-base tracking-[.12em]">{showNumber ? `5478 1842 6701 ${card.last4}` : `5478 •••• •••• ${card.last4}`}</p><div className="mt-4 flex justify-between font-mono text-[10px] text-white/75"><span>ALEXEY LEBEDEV</span><span>{card.expires}</span></div></div>{controls.frozen ? <span className="absolute right-5 top-20 inline-flex items-center gap-1 rounded-full bg-black/55 px-3 py-1 text-xs backdrop-blur"><Snowflake className="size-3" />{frozenLabel}</span> : null}</div>;
}
